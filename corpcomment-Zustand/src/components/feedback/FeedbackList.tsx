...
export default function FeedbackList() {
  ...
  // we comment that two lines out in favor of Zustand state management
  const isLoading = useFeedbackItemsStore((state) => state.isLoading);
  const errorMessage = useFeedbackItemsStore((state) => state.errorMessage);
  const filteredFeedbackItems = useFeedbackItemsStore(
    (state) => state.getFilteredFeedbackItems()
    // when we have derived state we should execute the function
  );
  // alt + shift + down arrow key - to duplicate
